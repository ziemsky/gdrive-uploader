...
class ProjectVersion private constructor(
     ...
     * * `commitOffset` - number of commits from the last one tagged with version tag; omitted if the current commit is tagged with version tag.
     * * `commitHash` - git commit hash; omitted if the current commit is tagged with version tag.
     * * `dirtyFlag` - indicates presence of uncommitted changes; omitted if there are no uncommitted changes.
     *
     * Examples:
     * * `1.2.3`
